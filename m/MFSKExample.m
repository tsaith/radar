...
% In example <FMCWExample.html Automotive Adaptive Cruise Control Using
% FMCW Technology>, an automotive radar system is designed to perform range
% estimation for an automatic cruise control system. In the latter part of
% that example, a triangle sweep FMCW waveform is used to simultaneously
% estimating range and speed of the target vehicle.
%
...
% km/h along the same direction as the radar and a truck at 60 meters away,
% traveling at 70 km/h in the opposite direction. The radar itself is
% traveling at 60 km/h.

rng(2015);

[sFMCWWav,sCar,sCarPlatform,sCarChannel,sTruck,sTruckPlatform,...
    sTruckChannel,sTx,sRx,sRadarPlatform,c,fc,lambda,fs,maxbeatfreq] = ...
    helperMFSKSystemSetup;
...
% up or down sweep takes 1 milliconds so each triangle sweep takes 2
% milliseconds. Note that only one triangle sweep is needed to perform the
% joint range and speed estimation.

Nsweep = 2;
xr = helperFMCWSimulate(Nsweep,sFMCWWav,sRadarPlatform,...
    {sCarPlatform,sTruckPlatform},sTx,{sCarChannel,sTruckChannel},...
    {sCar sTruck},sRx);
...
sweep_slope = sFMCWWav.SweepBandwidth/sFMCWWav.SweepTime;
...
numsamp_step = round(sMFSKWav.SampleRate*sMFSKWav.StepTime);
spectrogram(step(sMFSKWav),kaiser(numsamp_step),...
    0,512,sMFSKWav.SampleRate,'yaxis')
axis([0.02 0.04 0 5])
...
Nsweep = 1;
release(sCarChannel);
sCarChannel.SampleRate = sMFSKWav.SampleRate;
release(sTruckChannel);
sTruckChannel.SampleRate = sMFSKWav.SampleRate;
release(sRx);
sRx.SampleRate = sMFSKWav.SampleRate;

xr = helperFMCWSimulate(Nsweep,sMFSKWav,sRadarPlatform,...
    {sCarPlatform,sTruckPlatform},sTx,{sCarChannel,sTruckChannel},...
    {sCar sTruck},sRx);
...
fs_dechirp = 1/(2*sMFSKWav.StepTime);
...
sPeakDetector = phased.CFARDetector('ProbabilityFalseAlarm',1e-2,...
    'NumTrainingCells',8);
...
sweep_slope = sMFSKWav.SweepBandwidth/...
    (sMFSKWav.StepsPerSweep*sMFSKWav.StepTime);
temp = [1 sweep_slope;sMFSKWav.StepTime sMFSKWav.FrequencyOffset]\...
    [Fbeat phi/(2*pi)].';
